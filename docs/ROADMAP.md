...
Audio, recording, general file transfer, general multitouch input, and automatic clipboard synchronization are not implemented or scheduled. Map zoom mode uses one bounded synthetic pinch gesture for the mouse wheel. Explicit rich-content paste is available for compatible focused apps through the optional Android helper; it does not cover all file attachment fields. Each broader feature would need its own design and privacy review. Public/network hosting is outside the current local-only design.
