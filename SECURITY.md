...
Optional image/file paste requires the locally installed Android paste helper. A user gesture sends one item, up to 16 MiB, to the loopback bridge, which stages it in a restricted host temporary file and copies it to the configured phone. Host staging files are removed after the attempt. If Android file cleanup fails during a connection loss, the running service retains the exact device paths, verifies the configured phone again, and retries removal before another connection can proceed. A force-killed service or computer crash can interrupt that in-memory retry. The helper keeps at most four private files, pruning older files on a later paste; uninstall it to remove those files. Android clipboard history, receiving apps, and other software with device access may retain the pasted content. The helper's content provider grants read access for Android's paste operation and exposes no general file browse or write API. Local programs and ADB-authorized users remain inside the existing trust boundary and can control the phone. The bridge does not log file bytes or original host filenames.

Phone PIN entry holds digits only temporarily in the masked field and relay memory, sends standard digit key events, and does not intentionally log or persist them. It blocks clipboard actions on that field. Clearing the field is not guaranteed memory erasure; browser autofill, extensions, developer tools, or other local software remain outside this protection. Optional lock detection reports only an awake, unobscured phone lock screen; it cannot identify the authentication method, confirm a focused PIN field, or prove authentication succeeded. The read-only, bounded diagnostic output is discarded; only coarse lock-state changes reach the browser. The opt-in boolean is the only persisted PIN-panel setting.

Wireless pairing accepts a temporary code only from the active local browser controller through a fixed WebSocket message. The code is passed to `adb pair` through stdin, not command arguments, and is not intentionally logged or stored. Cancellation and connection changes discard late results. A successful ADB response is followed by configured-phone identity verification; neither that response nor the identity check proves that video rendered. JavaScript and ADB cannot guarantee memory erasure of a submitted code. The browser pairing form is not implemented yet.
