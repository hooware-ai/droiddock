...
Optional image/file paste requires the locally installed Android paste helper. A user gesture sends one item, up to 16 MiB, to the loopback bridge, which stages it in a restricted host temporary file and copies it to the configured phone. Temporary host and ADB transfer files are removed after the attempt; the helper keeps at most four private files, pruning older files on a later paste. Uninstall the helper to remove those files. Android clipboard history, receiving apps, and other software with device access may retain the pasted content. The helper's content provider grants read access for Android's paste operation and exposes no general file browse or write API. Local programs and ADB-authorized users remain inside the existing trust boundary and can control the phone. The bridge does not log file bytes or original host filenames.
