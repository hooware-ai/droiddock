...
Rotation resets the decoder for the new video dimensions. If this browser cannot decode the phone's H.264 configuration, DroidDock reports that instead of waiting for video. After a Wi-Fi interruption, restore connectivity and reconnect. Current stream settings are a maximum dimension of 1280 pixels, up to 60 fps, and 6 Mbps H.264; these are encoder settings, not performance guarantees.
