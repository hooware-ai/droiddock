...
- Open the three-dot panel for device details, appearance, and fallback text input. Appearance offers **System**, **Light**, and **Dark**; System is the default and follows the operating system. The choice is stored only in this browser and does not recolor or filter the phone video. Fallback text input uses scrcpy text injection, supports ASCII and some accented letters, and is limited to 300 UTF-8 bytes per send. A live `N / 300 bytes` counter uses the same UTF-8 check as Send; over-limit text stays in the field so it can be shortened.
