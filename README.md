...
If the configured phone cannot be reached, DroidDock may show a wireless pairing recovery hint. A `candidate` means one pairing advertisement matched the configured phone's connect advertisement, but the phone's permanent identity was **not** verified. `unknown` means the evidence was missing, ambiguous, contradictory, or unavailable. Neither state proves that pairing was lost. Check the physical phone and follow [wireless pairing recovery](docs/SETUP.md#wireless-debugging-android-11) if needed. The local bridge now has a controller-only pairing protocol, but the browser does not yet offer a pairing form.
